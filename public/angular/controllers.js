...
var AppCtrl, DataClass, DataElement, GameClock, GameDataCtrl, GuestTeam, HeaderController, HomeTeam, LowerThirds, RosterController, RouteController, Scoreboard, Team,
...
Team = (function(_super) {
  ...
  function Team($scope) {
    ...
    $scope.isCoach = function(player) {
      var error, foundCoach;
      foundCoach = false;
      try {
        angular.forEach($scope.positions, function(s) {
          if (player.position.indexOf(s.value) >= 0) {
            if (s.type === 1) {
              return foundCoach = true;
            }
          }
        });
        return foundCoach;
    ...
    $scope.usePositions = function() {
      var error, useThem;
      useThem = false;
      try {
        angular.forEach($scope.model.roster, function(p) {
          if ($scope.isPlayer(p)) {
